...
foreach($categorie->getAll() as $key => $value) {
    $pdf->AddPage();
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->SetWidths(array(190));
    $pdf->row(array($value['cat_naam']));
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->SetWidths(array(50, 35, 35, 35, 35));
    $pdf->row(array('Product naam','Telling 1', 'Telling 2', 'Telling 3', 'Telling 4'));
    foreach($product->getByCat($value['cat_id']) as $keyz => $valuez) {
        $pdf->SetFont('Arial', '', 8);
        $pdf->row(array($valuez['product_naam'], ' ', ' ', ' ', ' '));
    }
}
$pdf->AliasNbPages();
$pdf->Output();
